...
public class TraceErrorHandlerManager {
	...
	public TraceErrorHandlerManager(TraceErrorHandlerManagerConfig config, CacheManager cacheManager,
			TraceService traceService, MessageSource messageResource) {
		this.config = config;
	...
	public void start() throws Exception {
		try {
			...
			traceErrorHandler = new TraceErrorHandler(config.getName(), errorCache01, mergeCache, messageResource,
					traceService, delayForNoMessage, maxRetry, exceptionDelay);
	...
	public void stop() {
		...
		if (traceErrorHandler != null)
			traceErrorHandler.stop();
