...
public class MQTestConnectionController {
	...
	@RequestMapping(value = "/trace/test/wmqcon", params = "method=GET", method = RequestMethod.POST, headers = "content-type=application/json")
	public @ResponseBody ComMessage<Map<String, String>, String> conn(
		...
		if (isBind) {
			...
			if (channelName != null)
				envs.put(CMQC.CHANNEL_PROPERTY, channelName);
			if (hostName != null)
				envs.put(CMQC.HOST_NAME_PROPERTY, hostName);
			if (port != 0)
				envs.put(CMQC.PORT_PROPERTY, new Integer(port));
			if (userId != null)
				envs.put(CMQC.USER_ID_PROPERTY, userId);
			if (password != null)
				envs.put(CMQC.PASSWORD_PROPERTY, password);
		}
