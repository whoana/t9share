...
public class DefaultTraceMsgCreator implements TraceMsgCreator {
	...
	public MTEHeader create(
		...
		MTEHeader header = new MTEHeader();
		header.setMcdMsd("xml");
		header.setMcdSet("");
		header.setMcdType("");
		header.setMcdFmt("");
		// --------------------------------------------------------------------
		// a = "usr.mte_info.interface_info"
		// --------------------------------------------------------------------
		header.setaHostId(hostId);
		header.setaGroupId(groupId);
		header.setaIntfId(interfaceId);
		header.setaDate(date);
		header.setaTime(time);
		header.setaGlobalId(globalId);
		// --------------------------------------------------------------------
		// b = "usr.mte_info.host_info"
		// --------------------------------------------------------------------
		header.setbHostId(processHostId);
		header.setbOsType(processOsType);
		header.setbOsVersion(processOsVersion);
		// --------------------------------------------------------------------
		// c = "process_info"
		// --------------------------------------------------------------------
		String ptime = getNonoTime();
		header.setcDate(ptime.substring(0, 8));
		header.setcTime(ptime.substring(8));
		header.setcProcessMode(processMode);
		header.setcProcessType(processType);
		header.setcProcessId(processId);
		header.setcHubCnt(hubCnt);
		header.setcSpokeCnt(spokeCnt);
		header.setcRecvSpokeCnt(recvSpokeCnt);
		header.setcHopCnt(hopCnt);
		header.setcApplType(appType);
		// --------------------------------------------------------------------
		// d = "status_info"
		// --------------------------------------------------------------------
		header.setdStatus(status);
		header.setdErrorType("");
		header.setdErrorCode("");
		header.setdErrorReason("");
		header.setdErrorMessage("");
		header.setdErrorqMsgId("");
		header.setdTargetq("");
		// --------------------------------------------------------------------
		// e = "sender_info"
		// --------------------------------------------------------------------
		header.seteReplytoqmgr("");
		header.seteReplytoq("");
		header.seteFileName("");
		header.seteDirectory("");
		header.seteExtractPgm("");
		header.seteDescription("");
		// --------------------------------------------------------------------
		// f = "receiver_info"
		// --------------------------------------------------------------------
		header.setfHostId("");
		header.setfDirectory("");
		header.setfFileName("");
		header.setfUploadPgm("");
		// --------------------------------------------------------------------
		// g = "data_info"
		// --------------------------------------------------------------------
		header.setgRecordCnt(recordCnt);
		header.setgRecordSize(recordSize);
		header.setgDataSize(dataSize);
		header.setgDataCompress(dataCompressYn);
		header.setgCompressionMethod("");
		header.setgCompressionMode("");
		header.setgCompressionSize("");
		header.setgDataConversion("N");
		header.setgConvertedSize("");
		header.setgConvMode("");
