...
public class ChronicleCacheManagerProxy extends CacheManagerProxy {
	...
	public void initialize() throws Exception {
		...
		for (CacheConfig cfg : distributeCacheConfigs) {
			...
			ChronicleCacheProxy<String, Trace> distributeCache = createCache(diskPath, cfg, String.class, Trace.class,
					new Trace());
		...
		for (CacheConfig botCacheConfig : botCacheConfigs) {
			ChronicleCacheProxy<String, StateEvent> cache = createCache(diskPath, botCacheConfig, String.class,
					StateEvent.class, new StateEvent());
			botCaches.add(cache);
		}
		...
		if (interfaceCacheConfig != null)
			interfaceCache = createCache(diskPath, interfaceCacheConfig, String.class, InterfaceInfo.class,
					new InterfaceInfo());
		...
		if (unmatchCacheConfig != null)
			unmatchCache = createCache(diskPath, unmatchCacheConfig, String.class, Unmatch.class, new Unmatch());
		...
		if (systemErrorCacheConfig != null)
			systemErrorCache = createCache(diskPath, systemErrorCacheConfig, String.class, SystemError.class,
					new SystemError());
	 ...
	 * @throws IOException
	 */
	private <K, V> ChronicleCacheProxy<K, V> createCache(String diskPath, CacheConfig cfg, Class<K> keyClass,
			Class<V> valueClass, V defaultValue) throws IOException {
