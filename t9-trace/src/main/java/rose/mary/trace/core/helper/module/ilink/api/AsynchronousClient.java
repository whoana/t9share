...
public class AsynchronousClient {
	...
	// attachment.put("action", "w");
	// attachment.put("buffer", buffer);
	// channel.write(buffer, attachment, new CompletionHandler<Integer, Map<String,
	// Object>>() {
	//
	// @Override
	...
	//
	//
	//// byte propertyLengthArray[] = new
	// byte[ILMessage.messagePropertyLengthLength];
	...
	//// trace.succeed();
	////
	//// String propertyLengthString = new String(propertyLengthArray, 0,
	// ILMessage.messagePropertyLengthLength);
	//// propertyLengthString = ILMessage.truncateCString(propertyLengthString);
	//// int propertyLength = 0;
	//// if(propertyLengthString.length() > 0)
	//// propertyLength = Integer.parseInt(propertyLengthString);
	...
	// //property
	// byte[] propertyLenBytes = read(ILMessage.messagePropertyLengthLength);
	// String propertyLengthString = new String(propertyLenBytes, 0,
	// ILMessage.messagePropertyLengthLength);
	// propertyLengthString = ILMessage.truncateCString(propertyLengthString);
	// int propertyLength = 0;
	// if(propertyLengthString.length() > 0) {
	...
	// tmpTextMessage.setDataLength(textLenBytes);
	// byte[] textBytes = read(tmpTextMessage.getDataLength()) ;
	// tmpTextMessage.setText(new String(textBytes, 0,
	// tmpTextMessage.getDataLength()));
	// msg.setInnerMessage(tmpTextMessage);
	// break;
	//
	// case 2010 : //Inner message type is BYTES.
	...
	public Message getMessage() throws Exception {
		...
		// System.arraycopy(header, 0, data, 0, header.length);
		// System.arraycopy(body, 0, data, header.length, body.length);
		// System.arraycopy(newline, 0, data, header.length + body.length,
		// newline.length);
