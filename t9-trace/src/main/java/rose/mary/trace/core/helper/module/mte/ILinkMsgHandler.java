...
public class ILinkMsgHandler implements MsgHandler {
	 ...
	 * @param channelName
	 */
	public ILinkMsgHandler(String qmgrName, String hostName, int port, String channelName, String userId,
			String password) {
	...
	BytesMessage buildMessage(BytesMessage msg, String usr, String mcd, byte[] data) throws JMSException {
		...
		if (mcdData.length != 0)
			strucLength += 4 + mcdData.length;
		if (usrData.length != 0)
			strucLength += 4 + usrData.length;
		// -----------------------------------------------------------
	 ...
	 * 
	 * 
	 * javax.jms.JMSException: Failed to send data to the server:
	 * java.io.IOException: Broken pipe
	 * -- ILAPI FUNCTION TRACE --
	 * [17/09/2019 10:41:31.584 ILSession.sendILMessage(byte[] header, byte[] body)]
	 * ILSession.sendBytes: Try to send header.
	 * [17/09/2019 10:41:31.584 ILSession.sendILMessage(byte[] header, byte[] body)]
	 * ILSession.sendBytes: Caught exception.
	 * 
	 * at com.mococo.ILinkAPI.jms.ILSession.sendILMessage(ILSession.java:1454)
	 * at
	 * com.mococo.ILinkAPI.jms.ILMessageConsumer.receiveHelper(ILMessageConsumer.java:215)
	 * at
	 * com.mococo.ILinkAPI.jms.ILMessageConsumer.receive(ILMessageConsumer.java:132)
	 * at
	 * rose.mary.trace.helper.module.mte.ILinkMsgHandler.get(ILinkMsgHandler.java:163)
