...
public class APITest {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println(
					"send msg [" + (header.length + body.length) + "][" + new String(header) + new String(body) + "]");
			...
			test.listen(new MessageListener() {
				...
				public void onMessage(Object msg) throws Exception {
					...
					// System.out.println("receive msg [" + ((byte[])msg).length + "][" + new
					// String(((byte[])msg)) + "]");
					if (msg == null)
						return;

					System.out.println(
							"receive msg [" + ((byte[]) msg).length + "][" + new String(((byte[]) msg), 0, 25) + "]");

					ILinkMessage linkMessage = new ILinkMessage(0);
					linkMessage.setHeader((byte[]) msg);

					// ILinkReportMessage reportMessage = (ILinkReportMessage)
					// linkMessage.getContents();
					...
					try {
						if (isStop) {
							if (logger != null) {
								...
								if (systemout)
									System.out.println("socket client stopping.");
							}
						...
						if (res == -1) {
							if (logger != null) {
								...
								if (systemout)
									System.out.println("connecction closed on reading:" + res);
							}
						...
						//
						// if(logger != null) {
						// logger.debug( "{class:ksnbct2, method:listen, queueSize:" +
						// receiveQueue.size() + ", dataLength:" + data.length + ", data:" + new
						// String(data) + "}");
						// }else {
						// if(systemout) {
						// System.out.println("{class:ksnbct2, method:listen, queueSize:" +
						// receiveQueue.size() + ", dataLength:" + data.length + ", data:" + new
						// String(data) + "}");
						// }
						// }
						...
						if (logger != null) {
							...
							if (systemout)
								System.out.println(this.getClass().getSimpleName() + " listener thread stop.");
						}
						break;
	...
	public void stop() {
		...
		if (logger != null) {
			...
			if (systemout)
				System.out.println("The client socket will stop.");
		}
